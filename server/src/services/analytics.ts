...
export const analyticsService = {
  ...
  trackUsage(record: Omit<UsageRecord, 'timestamp'>) {
    const entry = { ...record, timestamp: new Date() };
    usageRecords.push(entry);
    if (usageRecords.length > 50000) usageRecords.splice(0, usageRecords.length - 50000);
  },
  ...
  getTokensByModel(days = 30) {
    ...
    const result: Record<string, { input: number; output: number; calls: number }> = {};
    for (const r of usageRecords) {
      if (r.timestamp < since) continue;
      if (!result[r.model]) result[r.model] = { input: 0, output: 0, calls: 0 };
  ...
  // DAU / WAU / MAU
  getActiveUsers(period: 'day' | 'week' | 'month' = 'day') {
    const ms = period === 'day' ? 86400_000 : period === 'week' ? 604800_000 : 2592000_000;
    const since = new Date(Date.now() - ms);
    const users = new Set(
      usageRecords.filter(r => r.timestamp >= since && r.userId).map(r => r.userId)
    );
    return users.size;
  },
  ...
  getTopUsers(limit = 20) {
    const map: Record<string, { tokens: number; calls: number; revenue: number }> = {};
    for (const r of usageRecords) {
      if (!r.userId) continue;
      if (!map[r.userId]) map[r.userId] = { tokens: 0, calls: 0, revenue: 0 };
  ...
  getErrorRate() {
    ...
    for (const r of usageRecords) {
      if (!map[r.model]) map[r.model] = { total: 0, errors: 0 };
      map[r.model].total += 1;
      if (!r.success) map[r.model].errors += 1;
    }
  ...
  getRevenueMetrics() {
    ...
    const tokenRevenue = (thisMonthTokens / 1_000_000) * 1.0; // $1 per 1M tokens markup
  ...
  seedDemoData() {
    ...
    for (let i = 0; i < 2000; i++) {
      ...
      const ts = new Date(now - daysAgo * 86400_000);
    ...
    for (let i = 0; i < 500; i++) {
      events.push({
        ...
        timestamp: new Date(now - Math.random() * 30 * 86400_000)
      });
    }
