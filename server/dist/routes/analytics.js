...
const logger = (0, logger_1.createLogger)('analytics-route');
// All analytics endpoints require auth (admin in production)
// For MVP we just require login
router.get('/overview', auth_1.requireAuth, (_req, res) => {
    try {
        const revenue = analytics_1.analyticsService.getRevenueMetrics();
        const funnel = analytics_1.analyticsService.getFunnelMetrics();
        const dau = analytics_1.analyticsService.getActiveUsers('day');
        const wau = analytics_1.analyticsService.getActiveUsers('week');
        const mau = analytics_1.analyticsService.getActiveUsers('month');
...
router.get('/models', auth_1.requireAuth, (_req, res) => {
    try {
        const byModel = analytics_1.analyticsService.getTokensByModel(30);
        const errorRates = analytics_1.analyticsService.getErrorRate();
        const latency = analytics_1.analyticsService.getLatencyStats();
...
router.get('/users/top', auth_1.requireAuth, (_req, res) => {
    try {
        const topUsers = analytics_1.analyticsService.getTopUsers(20);
        res.json({ topUsers });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.get('/hourly', auth_1.requireAuth, (_req, res) => {
    try {
        const hourly = analytics_1.analyticsService.getHourlyUsage();
        res.json({ hourly });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.get('/pricing/recommendations', auth_1.requireAuth, (_req, res) => {
    try {
        const byModel = analytics_1.analyticsService.getTokensByModel(30);
        const revenue = analytics_1.analyticsService.getRevenueMetrics();
        const topUsers = analytics_1.analyticsService.getTopUsers(100);
        // Identify power users (potential upgrade targets)
        const powerUserThreshold = 500000; // >500K tokens/month
        ...
        const modelCosts = {
            'deepseek-v3': 0.28 / 1000000,
            'deepseek-coder': 0.14 / 1000000,
            'glm-4': 0.36 / 1000000,
            'kimi-k2': 0.60 / 1000000
        };
